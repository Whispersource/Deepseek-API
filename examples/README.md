...
There is a single model, `deepseek-chat` — DeepSeek merged its Fast / Expert /
Vision modes on 2026-09-10, so the old `deepseek-expert` id is gone. See
[server/config.py](../server/config.py). DeepThink and web search are separate
toggles, requested via the `thinking` and `search` booleans in the request body —
or through `extra_body` with the OpenAI SDK.
